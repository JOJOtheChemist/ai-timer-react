...
class MessageDetailService:
    ...
    def get_message_detail(
        ...
        message_response.is_unread = main_message.is_read == 0
        ...
        for msg in context_messages:
            ...
            ctx_response.is_unread = msg.is_read == 0
        ...
        # 自动标记为已读（如果是接收方查看且未读）
        if main_message.receiver_id == user_id and main_message.is_read == 0:
            self._mark_as_read_async(db, message_id, user_id)
            detail_response.is_unread = False
            detail_response.is_read = True
    ...
    def get_tutor_feedback_history(
        ...
        messages = crud_message.get_multi_by_type(
            db, user_id, message_type="tutor", page=1, page_size=limit
        )[0]
        
        # 过滤特定导师的消息
        tutor_messages = [
            msg for msg in messages 
            if msg.related_id == tutor_id and msg.related_type == "tutor"
        ]
